...
RUN apt update \
    && apt install -y bash cmake curl gnupg gnupg2 libidn11-dev libsasl2-dev libldap2-dev libsasl2-dev libssl-dev \
...
RUN /bin/bash -l -c ". /etc/profile.d/rvm.sh && rvm install 2.6.1 && rvm use 2.6.1 --default && ruby -v && gem install aasm  && gem install rake  && gem install rugged && gem install rubocop && gem install pronto && gem install pronto-rubocop"
